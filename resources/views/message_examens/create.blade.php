...
<div class="blank"> 
    ...
    <div class="blankpage-main">
        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                <div class="row">
                    @foreach($examen->messages as $msg)    
                        ...
                        <div class="col-md-10 col-sm-10">
                          <div class="panel panel-default arrow left">
                            <div class="panel-body">
                              ...
                              {!! Form::open(['route' => ['messageExamens.destroy', $msg->id], 'method' => 'delete']) !!} 
                                <p class="text-right interaction">
                                <!-- Like section -->
                                @if(!$msg->liked())
                                @can('LIKE un message')
                                <a href="{{ route('like.examen.message',$msg->id) }}" class="btn btn-primary btn-sm like"><i class="fa fa-thumbs-up fa-lg"></i> J'aime <span class="badge badge-info">{{ $msg->likeCount }}</span></a>
                                @endcan
                                @else
                                @can('DISLIKE un message')
                                <a href="{{ route('dislike.examen.message',$msg->id) }}" class="btn btn-primary btn-sm like"><i class="fa fa-thumbs-down fa-lg"></i> Je n'aime pas ça <span class="badge badge-info">{{ $msg->likeCount }}</span></a>
                                @endcan
                                @endif
                                <!-- end Like section -->
                                @hasanyrole('Administrateur|Super administrateur|Enseignant')
                                    @can('Modifier un message')
                                        <a href="{{ route('messageExamens.edit', $msg->id) }}" class="btn btn-warning btn-sm"><i class="fa fa-edit fa-lg"></i> Modifier</a>
                                    @endcan
                ...
                <div class="row">
                    {!! Form::open(['route' => 'messageExamens.store']) !!}
